...
Class CConvertHXS
  Private oFol, oFils, oFil, oFols, oFol1
  Private iCount1, iCount2, iCountDone, Q2, s1, s2, s3, s4, i2, i3, BooRet, BooRetCHI, UB, iDex, sLog, Ret, cHAID
  Private ParFol, TempFol, DefFile
  Private LenTempPath '-- length of Path of files that needs to be removed from left side to get usable relative Path for compiler.
  Private HXSPath, HXTPath, HXKPath, HHPPath, HHCPath, HHKPath, HHCName, CHMPath, CHIPath, HHKName, CHMName, CHIName, CHMTitle, CHMTitleBarTitle, HXCPath
  Private FSOcc, TScc, SHcc, sPath_, oFol_, oFol2_, oFols_, oFils_, oFil_, AFils_, LnkPath_, i3_, UB2_
  Private DicKeys
  Private ParPath_  '--send in full parent Path to append to relative paths for accessing files.
  Private AFils(), A1, A2(), ATemp()
  Private AFolPathsLong(), AFolPathsShort(), FolsCount, sFolNameShort
  Private TYPE_LINE
  Private sWebFiles            '-- list of files for processing.

   '-- Settings:   
Dim Compiler, PathCompiler   '-- Required Path to hhc.exe, the HTML Help compiler.
...
Dim BooVerbose                    '-- Boolean. want messages while processing?  Default False. (Use for debugging.)
...
Dim CustomCSSCode              '-- optional custom CSS.
Dim MSCrapString, sBaseNameHXT, sHTMLPath, sBlankCode1, sBlankCode2, iBlankCount    '1/2018
Dim sParamName, sParamLocal, sParamEnd

Public Property Let CompilerPath(sVal)
  PathCompiler = sVal  '-- Path without quotes to check for valid Path.
  Compiler = Q2 & sVal & Q2
End Property

Public Property Let Zip7Path(sVal)
  SevZip = sVal
End Property

Public Property Let DestFolderPath(sVal)
  DestinationFolder = sVal
End Property

Public Property Let DeleteTemp(BVal)
  BooDeleteTemp = BVal
End Property

Public Property Let GetTitle(BVal)
   BooGetTitle = BVal
End Property

Public Property Let CreateCHI(BVal)
   BooCreateCHI = BVal
End Property

Public Property Let Verbose(BVal)
  BooVerbose = BVal
End Property

Public Property Let CleanScript(BVal)
  CleanScriptMess = BVal
End Property

Public Property Let CustomBackColor(sVal)
  BackColor = sVal
End Property

Public Property Let CustomFontColor(sVal)
  FontColor = sVal
End Property

Public Property Let CustomCSS(sVal)
  CustomCSSCode = sVal
End Property
...
Private Sub Class_Initialize()
  ...
  sParamEnd = Q2 & ">" & vbCrLf
  iBlankCount = 0
  Set FSOcc = CreateObject("Scripting.FileSystemObject")
  Set DicKeys = CreateObject("Scripting.Dictionary")
  Set SHcc = CreateObject("WScript.Shell")
End Sub
...
'  For each HXS file Path, do the following:
...
' before compiling, so if Run does not successfully wait for compiler return then CHM will be open when FSOcc.MoveFile
' is called and that call will fail with Permission Denied.


 '-- 0-ok. 1-7zip or compiler missing.
Public Function ProcHXS(sFilePath)
  Dim Pt1
    'reset variables.
       sHTMLPath = ""
       iBlankCount = 0
    ...
    ParFol = Left(sFilePath, (Pt1 - 1))
    TempFol = ParFol & "\Temp1"
    If FSOcc.FolderExists(TempFol) = True Then FSOcc.DeleteFolder TempFol, True
    LenTempPath = Len(TempFol & "\")
    If FSOcc.FolderExists(DestinationFolder) = False Then DestinationFolder = ParFol

  'DefFile = ""  '-- clear last default file and reset for this op.

   If (FSOcc.FileExists(PathCompiler) = False) Or (FSOcc.FileExists(SevZip) = False) Then ProcHXS = 1: Exit Function
   
   BooRet = ProcessHXSFile(sFilePath) '-- unpack HXS with 7-Zip and make sure there's at least a .HXT file present.
    If BooRet = False Then 
      ProcHXS = 2  '-- Likely cause: A problem with 7-Zip or no .HXT file in HXS.
      If BooDeleteTemp = True Then CleanUp
      Exit Function  
    End If   
        '-- found unpacked TOC file, at least.
    
    If BooVerbose = True Then MsgBox "Finished unpacking HXS file. Proceeding with functions to conver project files for CHM."
       
    ConvertHXTtoHHC  ' convert HXT to HHC, which will then be HHCPath.   
       If FSOcc.FileExists(HXKPath) = True Then
  ...
  '-- the contents and index files also need to be fixed. (.HHC and .HHK)
    ...
    ProcessLinksForCHM sWebFiles  '-- (This operation also calls the function to remove script and adjust CSS if CleanScriptMess = True.)
               
        '-- Make sure everything is ready to go.
    If FSOcc.FileExists(HHPPath) = False Then ProcHXS = 3: If BooDeleteTemp = True Then CleanUp: Exit Function  '-- Project file missing. HHP
    If FSOcc.FileExists(HHKPath) = False Then ProcHXS = 4: If BooDeleteTemp = True Then CleanUp: Exit Function  '--Index file file missing. HHK
    If FSOcc.FileExists(HHCPath) = False Then ProcHXS = 5: If BooDeleteTemp = True Then CleanUp: Exit Function  '-- TOC file file missing. HHC

      '-- All done with processing of HXS. Send it to the compiler. 
       If BooVerbose Then MsgBox "Done processing. Calling HTML Help Workshop compiler." 
          
      Ret = SHcc.Run(Compiler & " " & Q2 & HHPPath & Q2, 0, True)
   ...
   For i3 = 1 to 100   '-- copy CHM to parent folder as soon as possible. This should not actually Run more than 1 loop, since code above waits for return.
      SleepX "500"
     If FSOcc.FileExists(CHMPath) = True Then
        If FSOcc.FileExists(DestinationFolder & "\" & CHMName) = True Then FSOcc.DeleteFile DestinationFolder & "\" & CHMName, True
        FSOcc.MoveFile CHMPath, DestinationFolder & "\" & CHMName
        iCountDone = iCountDone + 1
        BooRet = True
        If BooRetCHI = True Then Exit For
     End If
     If BooCreateCHI = True Then
        If FSOcc.FileExists(DestinationFolder & "\" & CHIName) = True Then FSOcc.DeleteFile DestinationFolder & "\" & CHIName, True
        FSOcc.MoveFile CHIPath, DestinationFolder & "\" & CHIName
        BooRetCHI = True
        If BooRet = True Then Exit For
     End If 
   Next    
     If BooRet = False Then
         ProcHXS = 6  '-- "No CHM file found. compiler must have failed.
     Else     
         ProcHXS = 0
     End If     
        If BooDeleteTemp = True Then CleanUp
...
Private Function ProcessHXSFile(sFil)
Dim Boo
 On Error Resume Next
     If FSOcc.FolderExists(TempFol) = True Then FSOcc.DeleteFolder TempFol, True
       Set oFol = FSOcc.CreateFolder(TempFol)
     Set oFol = Nothing
...
Private Function GetHXTandHXKPaths()
  Dim sExt1, sLinkName
   HXTPath = ""
   HXKPath = "" 
   GetHXTandHXKPaths = False

  Set oFol = FSOcc.GetFolder(TempFol)
    Set oFils = oFol.Files
     ...
     Set oFils = Nothing
  Set oFol = Nothing
     If FSOcc.FileExists(HXTPath) = True Then GetHXTandHXKPaths = True 
 End Function
         
'------------------------------ convert HXT(TOC) to HHC(TOC) file ------------------------------------------
Private Sub ConvertHXTtoHHC()
  Dim Pt3, i6
...
' this first bit gets the file base name of the hxt, which is the best guess as
...
s1 = Join(A2, "")
  ...
  '1/2018 - Get the HXT base name to check for faulty paths using "ms-help:"
   '-- example: the script56 help file has script56.hxt. It's not clear
   '-- whether that's a required pattern, but it seems to at least be standard.
   
s2 = Left(HXTPath, (len(HXTPath) - 4))
HHCPath = s2 & ".hhc"
HHCName = ""
Pt3 = InStrRev(s2, "\")
HHCName = Right(s2, (len(s2) - Pt3))  'blah.hhc

WriteFile HHCPath, s1
 End Sub

' deal with invalid HXT file that has no Title attributes.
' They'll have to be retrieved from the actual webpages.

  '-- Convert an HXT listing to an HHC listing. iType is 1 if this is an item, 2 if it's a header.
Private Function GetItemHXT(sLine, HXTFolPath)  '1/2018
...
'-- write a page here and put the topic text in it.
  ...
  '-- if there's no URL but the HTML files location has been found, make a blank page.
   If Len(sURL) = 0 And Len(sHTMLPath) > 1 Then
       ...
       WriteFile BlankPath, sBlankCode1 & sTitle & sBlankCode2 ' write a blank page with the title as a heading.
   End If 
...
HHKName = Right(s2, (len(s2) - Pt3))
 ...
 WriteFile HHKPath, s1
End Sub

  '-- Convert an HXK listing to an HHK listing. 
 Private Function GetItemHXK(sKey, sJump)
...
End Sub
 ...
 Private Function GetItemHHCtoHHK(sKey, sJump)
   Dim Pt1, Pt2, sTitle, sURL
   On Error Resume Next
    GetItemHHCtoHHK = ""
...
Private Sub ParseHXC()
 Dim s7, A7, i7, Pt71, Pt72
 On Error Resume Next
     If FSOcc.FileExists(HXCPath) = False Then Exit Sub
    s7 = ReadFile(HXCPath)
    s7 = Replace(s7, Chr(9), "")
  A7 = Split(s7, vbCrLf)
...
Private Sub CreateHHP()
  Dim sExt2
  sWebFiles = ""
       '-- this is all added to deal with the problem of extra long folder names in some help2 projects.
     FolsCount = 0
     sFolNameShort = "vkq_z"
...
Private Sub RepairFolderTreeHelper(sFolder)
  Dim oFols2, oFol2, oFol3, sPath3
    On Error Resume Next
    Set oFol2 = FSOcc.GetFolder(sFolder)
...
Private Function GetFileList(FolPath, ParFolList)
  Dim SubPath, oFols2, sList, oFol2, oFol3, oFils2, oFil2, ParFol1
    Set oFol2 = FSOcc.GetFolder(FolPath)
...
Private Sub GetDefFile(FileList)
 Dim AList, i8, s8
   On Error Resume Next
...
Private Sub UnZipHXS(sCom)
  Dim Ret, Pt4, ScrPath
   On Error Resume Next
    ScrPath = WScript.ScriptFullName
      Pt4 = InStrRev(ScrPath, "\")
    ScrPath = Left(ScrPath, Pt4) & "zip7.vbs"
    SHcc.Run Q2 & ScrPath & Q2 &  " " & Q2 & sCom & Q2, , True
End Sub

Private Sub CleanUp()
  On Error Resume Next
    If BooDeleteTemp = False Then Exit Sub
   SleepX "4000"    ' pause before trying to delete folders.
    If FSOcc.FolderExists(TempFol) Then FSOcc.DeleteFolder TempFol, True
    sHTMLPath = ""
End Sub  

'-- call this with file list, but first assign parent Path, which must be the first part of full Path. Maybe TempFol & "\"?
Private Sub ProcessLinksForCHM(sFilesList)
    Dim sFil1
  AFils_ = Split(sFilesList, vbCrLf)
  UB2_ = UBound(AFils_)

   If BooVerbose = True Then MsgBox "Beginning processing of web files for CHM. Total files to process: " & CStr(UB2_ + 1)
...
'-- At this point the only major job left is replacing the MSHelp links, but that's a big job.
    If BooVerbose = True Then MsgBox "Beginning phase 2 with web files for CHM -- replacing MSHelp links with normal links. Total files to process: " & CStr(UB2_ + 1)
...
Private Sub FLProcessFile(sPath_, Phase)
  Dim s1_, s2_, s3_, sDisp, LenDisp, Pt1_, Pt2_, Pt3_, Pt4_, sMid, sKey, i2_, i4_, sLnk, UBTemp, sUCase
    ...
    Else  ' second phase: go through files again and fix links.
       ...
       If Len(CustomCSSCode) > 0 Then s1_ = AddCustomStyle(s1_)
...
Private Function AddCustomStyle(sFilText)
  Dim Pt1_, Pt2_
   Pt1_ = InStr(1, sFilText, "</HEAD", 1)
     If Pt1_ > 0 Then
        AddCustomStyle = Left(sFilText, (Pt1_ - 1)) & CustomCSSCode & Right(sFilText, (len(sFilText) - (Pt1_ - 1)))  
     Else
...
Public Function ReadFile(Path)
  Set TScc = FSOcc.OpenTextFile(Path, 1)
    ReadFile = TScc.ReadAll
    TScc.Close
  Set TScc = Nothing
End Function

Public Sub WriteFile(Path, sContent)
  Set TScc = FSOcc.CreateTextFile(Path, True)
    TScc.Write sContent
    TScc.Close
  Set TScc = Nothing
End Sub

Private Sub SleepX(MSecs) '--Function to cause A pause since WScript is Not available from webpage:
  Dim Ret1
    On Error Resume Next
